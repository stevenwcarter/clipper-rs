...
fn main() -> Result<()> {
    ...
    println!("   ✅ Processed in {:?}, {} dimensions", time_file, embedding_file.len());
    ...
    println!("   ✅ Processed in {:?}, {} dimensions", time_dynamic, embedding_dynamic.len());
    ...
    println!("   ✅ Processed in {:?}, {} dimensions", time_bytes, embedding_bytes.len());

    // Verify consistency
    let sim1 = cosine_similarity(&embedding_file, &embedding_dynamic);
    let sim2 = cosine_similarity(&embedding_file, &embedding_bytes);
    println!("   📊 Consistency: File↔Dynamic={:.6}, File↔Bytes={:.6}", sim1, sim2);
    assert!(sim1 > 0.999 && sim2 > 0.999, "Single image methods should be identical");
    ...
    println!("   ✅ Processed {} images in {:?}", batch_embeddings.len(), time_batch);
    ...
    println!("   ✅ Processed {} DynamicImages in {:?}", batch_dynamic.len(), time_batch_dynamic);
    ...
    println!("   ✅ Processed {} byte arrays in {:?}", batch_bytes.len(), time_batch_bytes);
    ...
    println!("   📊 Batch consistency: Paths↔Dynamic={:.6}, Paths↔Bytes={:.6}", batch_sim1, batch_sim2);
    assert!(batch_sim1 > 0.999 && batch_sim2 > 0.999, "Batch methods should be identical");

    // Verify batch vs individual consistency
    let individual_vs_batch = cosine_similarity(&embedding_file, &batch_embeddings[0]);
    println!("   📊 Individual↔Batch consistency: {:.6}", individual_vs_batch);
    assert!(individual_vs_batch > 0.999, "Batch should match individual processing");
    ...
    for (i, text) in texts.iter().enumerate() {
        ...
        println!("{}️⃣  '{}' → {} dims in {:?}", 
                 i + 1, text, text_embeddings[i].len(), time_text);
    }
    ...
    let best_match_idx = text_embeddings.iter()
        .enumerate()
        .map(|(i, emb)| (i, cosine_similarity(&batch_embeddings[1], emb)))
        .max_by(|a, b| a.1.partial_cmp(&b.1).unwrap())
        .unwrap();
    
    println!("   🎯 Best match: '{}' (similarity: {:.4})", 
    ...
    if batch_efficiency > 0.0 {
        println!("📈 Batch processing is {:.1}% more efficient than individual calls", batch_efficiency);
    }
